#ifdef ESP32
#include <stdio.h>
/* application.h is Esp32's standard library. Defines the Arduino String
 * object, the Arduino delay() procedure, and the Esp32 TCPClient. */
#include "ESP32AWSImplementations.h"
...
char* Esp32HttpClient::send(const char* request, const char* serverUrl, int port) {
...
bool Esp32HttpClient::usesCurl() {
    /* Does not use curl command. */
    return false;
}

Esp32DateTimeProvider::Esp32DateTimeProvider() {
}

const char* Esp32DateTimeProvider::getDateTime() {
    return updateCurTime();
}
bool Esp32DateTimeProvider::syncTakesArg(void) {
    return true;
}

void Esp32DateTimeProvider::sync(const char* dateTime) {
